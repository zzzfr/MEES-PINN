...
class Disk(Geometry):
    def __init__(self, center, radius):
        self.center = jnp.array(center, dtype=config.real(jnp))
        self.radius = radius
    ...
    def distance2boundary_unitdirn(self, x, dirn):
        ...
        xc = x - self.center
        ad = jnp.dot(xc, dirn)
        return (-ad + (ad ** 2 - jnp.sum(xc * xc, axis=-1) + self._r2) ** 0.5).astype(config.real(jnp))
    ...
    def uniform_boundary_points(self, n):
        ...
        X = jnp.vstack((np.cos(theta), jnp.sin(theta))).T
    ...
    def background_points(self, x, dirn, dist2npt, shift):
        ...
        n = max(dist2npt(dx), 1)
        h = dx / n
        pts = x - jnp.arange(-shift, n - shift + 1, dtype=config.real(jnp))[:, None] * h * dirn
        return pts
